...
class QuizBloc extends Bloc<QuizEvent, QuizState> {
  ...
  QuizBloc(this.repository) : super(QuizInitial()) {
    on<LoadQuestionsByCategory>((event, emit) async {
      emit(QuizLoading());
      try {
        final questions = await repository.fetchQuestionsByCategory(event.category);
    ...
    on<AnswerQuestion>((event, emit) async {
      if (state is QuizLoaded) {
        ...
        final isCorrect = event.selectedIndex == current.questions[current.currentIndex].correctIndex;
        ...
        if (!current.answered) {
          ...
          final answered = await AppSettingsSharedPreferences().getAnswered(category);
        }
